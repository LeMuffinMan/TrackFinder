...
impl TrailArchive {
    ...
    pub fn pump(&mut self, net: &mut crate::trails::TrailNetwork) -> bool {
        ...
        for message in arrived {
            match message {
                Message::Manifest(Ok(body)) => {
                    // A dev server answers 200 with `index.html` for anything it
                    // does not have, so a missing manifest arrives as a page, not
                    // as a 404. Parsing that yields "expected value at line 1
                    // column 1", which says nothing about the actual problem.
                    if body.trim_start().starts_with('<') {
                        self.manifest_state = Fetch::Failed;
                        self.last_error = Some(format!(
                            "no trail data at {}{} — generate it with trailprep",
                            self.base, MANIFEST_FILE
                        ));
                        continue;
                    }
...
mod tests {
    ...
    fn a_missing_manifest_says_so_in_words() {
        ...
        store.inbox.lock().unwrap().push(Message::Manifest(Ok(
            "<!DOCTYPE html>\n<html lang=\"en\">".to_owned(),
        )));
        let mut net = crate::trails::TrailNetwork::default();
        store.pump(&mut net);

        let err = store.last_error.clone().expect("an error");
        assert!(err.contains("no trail data"), "{err}");
        assert!(err.contains("http://localhost:8080/trails/regions.json"), "{err}");
        assert!(err.contains("trailprep"), "the message must say what to do: {err}");
        assert!(!store.ready());
    }
